...
class Gamepad:
  ...
  def _handle_button_event(self, button, value):
    for h in self._handlers:
      if h.event == common_names[button] or h.event == common_names[button] + ":pressed":
        h(value, h.event)
  ...
  def _read_device(self):
    ...
    if (event_buf):
      timestamp, value, event_type, number = struct.unpack("IhBB", event_buf)
  ...
  def _update_connection(self):
    if not self._device or not os.path.exists(self._device) or not self._connected:
